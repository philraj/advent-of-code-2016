...
const sectorSum = input.reduce((sum, line) => {
  const { name, sectorID, checksum } = splitLine(line);

  const splitName = name.replace(/-/g, '').split('');
  const dict = generateDict(splitName);
  const sortedKeys = sortDict(dict);

  return roomIsValid(sortedKeys, checksum) ?
    sum + sectorID :
    sum;
}, 0);
...
// generate dictionary with chars mapped to char counts
function generateDict(chars) {
  return chars.reduce((dict, char) => {
    if (dict[char])
      dict[char]++;
    else
      dict[char] = 1;
...
function roomIsValid(sortedKeys, checksum) {
  return checksum.split('').every((char, i) => {
    return char === sortedKeys[i];
  });
}
...
const northPoleRoom = input.reduce((foundID, line) => {
  const { name, sectorID } = splitLine(line);
  const nameDecrypted = name
    .replace(/-/g, ' ')
    .split('')
    .map(rotate(sectorID)) // rotate letters by sectorID
    .join('');

  if (nameDecrypted.indexOf('north') > -1)
    return sectorID;
  else
    return foundID;
}, null);

// return a func that rotates characters by sectorID amount
function rotate(sectorID) {
  return char => {
    if (char === ' ')
      return char;
    else
      return String.fromCharCode(
        (char.charCodeAt() - 97 + sectorID) % 26 + 97
      );
  }
}
